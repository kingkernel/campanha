...
class eleitorForms {
	...
	public function cadastroIn(){
		...
		$submit = '<input type="submit" value="cadastrar" class="btn btn-primary pull-right" style="margin:10px" id="cadEleitorBtn">';
			...
			$this->cadIn[0] = $nome->html().$city->html().$bairro->html().$rua->html().$numero->html();
			$this->cadIn[1] = $email->html().$fone1->html().$fone2->html().$zap->html().$face->html().$hidden1.$submit;
			$this->cadIn[2] = '$("#id_'.$fone1->inputName.'").mask(\'(99) 99999-9999\');$("#id_'.$fone2->inputName.'").mask(\'(99) 99999-9999\');$("#id_'.$zap->inputName.'").mask(\'(99) 99999-9999\');$("#cadEleitorBtn").click(function(){/* variaveis */var nome = $("#id_'.$nome->inputName.'").val();var cidade = $("#id_'.$city->inputName.'").val();var bairro = $("#id_'.$bairro->inputName.'").val();var rua = $("#id_'.$rua->inputName.'").val();var numero = $("#id_'.$numero->inputName.'").val();var email = $("#id_'.$email->inputName.'").val();var fone1 = $("#id_'.$fone1->inputName.'").val();var fone2 = $("#id_'.$fone2->inputName.'").val();var zap = $("#id_'.$zap->inputName.'").val();var face = $("#id_'.$face->inputName.'").val();var hidden1 = "'.$_SESSION["userinfo"]["idlicenca"].'";/* autenticação de campos *//* nome do eleitor */if(nome == ""||nome==null){alert("é preciso o nome do eleitor");$("#id_'.$nome->inputName.'").addClass("white label-warning");$("#id_'.$nome->inputName.'").focus();return false;};/* cidade */if(cidade == ""|| cidade ==null){alert("Qual a cidade do Eleitor?");$("#id_'.$city->inputName.'").addClass("white label-warning");$("#id_'.$city->inputName.'").focus();return false;}; /* envio de dados */$("#ajax").load("'.(urlcss($_GET)).'grava/eleitor/",{eleitor: nome,cidade: cidade, licenca:hidden1});});';
			return $this->cadIn;
	}

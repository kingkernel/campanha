...
--		FROM eleitores
--			INNER JOIN usuarios
--				ON (eleitores.licenca = usuarios.licenca)
--		WHERE
--		(eleitores.email=arg_email and eleitores.senha=arg_snhpwd and eleitores.acesso=1)
--		OR
--		(usuarios.email=arg_email and usuarios.snhpwd=arg_snhpwd and usuarios.ativo=1);
	end //
delimiter ;
...
delimiter //
	create procedure sp_login_e(arg_email varchar(50), arg_snhpwd varchar(64))
		begin
			select count(*) from eleitores where email=arg_email and senha=arg_snhpwd and acesso=1;
		end //
delimiter ;
...
-- select usuarios.id, usuarios.email, usuarios.snhpwd, usuarios.nome, usuarios.licenca as idlicenca, licencas.licenca, licencas.nomelicenca, licencas.dataexpiracao, usuarios.ativo from usuarios usuarios inner join licencas licencas on (usuarios.licenca = licencas.id) where usuarios.email=arg_usuario;
	SELECT eleitores.licenca,
       eleitores.nomeeleitor,
       eleitores.id,
       usuarios.id,
       usuarios.email,
       usuarios.nome,
       licencas.dataexpiracao,
       licencas.nomelicenca,
       licencas.id as idlicenca
FROM (usuarios usuarios
      INNER JOIN licencas licencas
         ON (usuarios.licenca = licencas.id))
     INNER JOIN eleitores eleitores
        ON     (eleitores.licenca = licencas.id)
           AND (eleitores.licenca = usuarios.licenca)
		WHERE (usuarios.email=arg_usuario) 
		OR (eleitores.email=arg_usuario);
